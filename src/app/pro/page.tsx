...
const ProPage = () => {
  ...
  const handlePlanSelection = async (planId: "month" | "year") => {
    if (!user) {
      return toast.error("Please login to select a plan.", {
        id: "login-error",
        position: "top-center",
        duration: 3000,
      });
    ...
    try {
      ...
      if (error.message.includes("Rate limit exceeded")) {
        ...
        toast.error("There was an error initiating your purchase. Try Again!");
      }
      console.log(error);
    } finally {
      setLoadingPlan("");
      ...
      {isUserLoaded && userSubscription?.status === "active" && (
        <div className="bg-blue-50 border-l-4 border-blue-500 p-4 mb-8 rounded-md">
          <p className="text-blue-700">
            You have an active{" "}
            <span className="font-semibold">{userSubscription.planType}ly</span>
          </p>
        </div>
      )}
              ...
              <ul className="space-y-3">
                {plan.features.map((feature, fIdx) => (
                  <li key={fIdx} className="flex items-center">
                    <Check
                      className={`size-5 ${plan.highlighted ? "text-purple-500" : "text-green-500"} mr-2 flex-shrink-0`}
                    />
